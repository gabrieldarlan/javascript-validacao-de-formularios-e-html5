...
const mensagensDeErro = {
  ...
  dataNascimento: {
    ...
    customError: `Você deve ser maior de idade, para efetuar o cadastro.`
  },
  cpf: {
  ...
  cep: {
    ...
    customError: `Não foi possível efetuar a busca do CEP.`
  },
  logradouro: {
...
function validaCPF(input) {
  const cpfFormtado = input.value.replace(/\D/g, '')
  let mensagem = ``

  if (!checarCPFRepedito(cpfFormtado) || !checaEstrutraCPF(cpfFormtado)) {
...
function recuperarCEP(input) {
  ...
  if (!input.validity.patternMismatch && !input.validity.valueMissing) {
    fetch(url, options)
      ...
      .then(data => {
        if (data.erro) {
          input.setCustomValidity(`Não foi possível efetuar a busca do CEP`)
          return
        }
        input.setCustomValidity(``)
        preencheCampoCEP(data)
        return
      })
  }
}

function preencheCampoCEP(data) {
